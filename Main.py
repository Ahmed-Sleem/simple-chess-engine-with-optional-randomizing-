...
class Main:
    ...
    def playHumanMove(self):
        try:
            ...
            print("""To undo the inputed last move, type "undo".""")
            #get human move
            play = input("oponent move: ")
            if (play=="undo"):
                self.board.pop()
                self.board.pop()
                self.playHumanMove()
    ...
    def playEngineMove(self, maxDepth, color):
        engine = ce.Engine(self.board, maxDepth, color, int(input("1 for randomizing 0 for not ")))
    ...
    def startGame(self):
        ...
        while(color!="b" and color!="w"):
            color = input("""the oponent Play as (type "b" or "w"): """)
        maxDepth=None
